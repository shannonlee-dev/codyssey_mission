...
### 질문 1. 퀴즈 데이터가 1000개 이상으로 늘어나면 현재 JSON 저장 방식의 한계는 무엇인가

현재 방식은 학습 과제 규모에서는 충분하지만, 퀴즈가 1000개 이상으로 커지면 아래 한계가 생길 수 있다.

#### 1) 전체 파일을 매번 다시 읽고 다시 써야 한다
현재 구조는 프로그램 시작 시 `state.json` 전체를 한 번에 읽고, 저장할 때도 전체를 다시 쓴다.  
데이터가 많아질수록 읽기/쓰기 시간이 길어지고 비효율이 커진다.

#### 2) 메모리에 전체 데이터를 한 번에 올린다
현재는 시작 시 퀴즈 전체를 모두 `Quiz` 객체로 복원한다.  
1000개 이상이면 메모리 사용량과 초기 로딩 비용이 커질 수 있다.

#### 3) 부분 수정이 비효율적이다
예를 들어 퀴즈 1개만 추가하거나 최고 점수만 바꿔도 전체 JSON 파일을 다시 저장한다.  
데이터가 커질수록 작은 수정에도 비용이 커진다.

#### 4) 동시 수정이나 충돌에 약하다
JSON 파일 하나에 전체 상태를 몰아두는 방식은 여러 프로세스/사용자가 동시에 접근하는 상황에 적합하지 않다.  
파일 단위 충돌이나 손상 가능성이 커질 수 있다.

#### 5) 검색/정렬/분석 기능 확장에 불리하다
나중에 카테고리별 검색, 난이도 필터, 최근 기록 조회 같은 기능이 필요해지면 JSON 한 파일 구조는 점점 불편해진다.

즉, 현재 JSON 방식은 **작은 콘솔 프로그램에서는 단순하고 이해하기 좋지만, 데이터가 커질수록 전체 읽기/전체 쓰기 구조의 성능과 관리 한계가 드러나는 방식**이다.

### 질문 2. JSON 손상 시 기본 데이터로 복구한다는 전략의 의미는 무엇인가

이 전략의 핵심 의미는 **프로그램 가용성을 우선 보장한다**는 것이다.

손상된 상태 파일을 보고 프로그램이 바로 죽어버리면 사용자는 아무 기능도 못 쓴다.  
반면 기본 데이터로 복구하면 기존 데이터 일부는 잃을 수 있어도 프로그램 자체는 계속 사용할 수 있다.

즉, 이 전략은 아래 우선순위를 선택한 것이다.
...
### 질문 3. 요구사항이 바뀌면 어떤 파일/클래스/메서드를 먼저 수정해야 하는가

요구사항이 무엇으로 바뀌는지에 따라 먼저 보는 위치가 달라진다.

#### 경우 1. 퀴즈 1개 구조가 바뀌는 경우
예: 힌트 필드 추가, 선택지를 5개로 변경, 정답 형식을 문자열로 변경
...
- `quiz.py`
  ...
  - `to_dict`
  - `from_dict`
  - 필요하면 `display`, `is_correct`
- 그 다음 `quiz_game.py`
  - `add_quiz`
  - `play_quiz`
  - 입력 검증 범위

즉, **퀴즈 데이터 구조가 바뀌면 먼저 `Quiz`를 보고, 그 구조를 사용하는 게임 흐름을 뒤따라 수정**한다.

#### 경우 2. 메뉴 기능이 바뀌는 경우
예: 퀴즈 삭제 기능 추가, 기록 히스토리 보기 추가
...
- `main.py`
  - 새 메뉴 번호 연결
- 필요하면 `state.json` 구조 확장

즉, **메뉴/흐름 요구사항은 먼저 `QuizGame`과 `main.py`를 본다.**

#### 경우 3. 저장 구조가 바뀌는 경우
예: 점수 히스토리 추가, 카테고리 저장 추가
...
- `quiz_game.py`
  ...
  - `save_state`
- `quiz.py`
  - 퀴즈 개별 필드가 바뀐다면 `to_dict`, `from_dict`
- `README`
  - 데이터 파일 설명, 스키마 설명 수정

즉, **파일 저장 규칙이 바뀌면 먼저 `load_state`와 `save_state`를 중심으로 본다.**

#### 경우 4. 입력 규칙이 바뀌는 경우
예: 메뉴 번호 범위 변경, 정답 입력 방식 변경
